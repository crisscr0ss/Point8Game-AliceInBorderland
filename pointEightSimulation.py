#This code is just a Simulation using the random library of a number guessing game 
# with logic inspired from the show Alice in Borderland[S2E5](Queen of Spades Game).
...
def MainLogic(death: int,PlArr: list,points: list):
    ...
    while(len(PlArr)>1):
        while(min(points)>death):
            ...
            t.sleep(2) #change to speed up output process
            x+=1
        ...
        if len(PlArr)>1:
            t.sleep(3) #change t speed up output process
